...
def resize_image(image, size=512, inter=cv2.INTER_AREA):
    '''
    Image resizing for neural network.
    This method scale image proportionally and locate it on the new image with size x size
    @param image: source image
    @param size: target size
    @param inter: interpolation method (cv2.INTER_AREA by default)
    @return: resized image as numpy format
    '''
